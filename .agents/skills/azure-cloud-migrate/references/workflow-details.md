...
1. **Resource-level status table** — After submitting a deployment, poll `az resource list` or `az deployment operation group list` and present a status table:
   ...
   ```
2. **Explain what's slow** — If a resource takes >2 minutes (e.g., Container Apps Environment with VNet), tell the user _why_ ("VNet integration provisions internal load balancers and DNS — this typically takes 3-5 min").
