...
} from "../lib/extract-claims.mjs";
import {
  gradeRecommendation,
  applyQualityFloor,
} from "../lib/grade-recommendation.mjs";
...
async function main() {
  ...
  if (!Array.isArray(recs)) {
    console.error(
      "[verify-and-regen] FATAL: recommendations.json must be an array of rec objects"
    );
    process.exit(2);
  }
  ...
  const rootResult = await resolveRepoRoot(
    recs,
    args.repoRoot,
    process.cwd(),
    signals
  ...
  if (rootResult.source === "api") {
    log(
      `repo-root from Vercel API: '${repoRoot}' (rootDirectory='${rootResult.apiOffset}')`
    );
  ...
  for (let i = 0; i < recs.length; i++) {
    ...
    if (rec?.abstain === true) {
      ...
      if (
        rec.observation &&
        typeof rec.observation === "object" &&
        rec.observation.summary
      ) {
        observations.push({
          index: i,
          candidateRef: rec.candidateRef ?? null,
    ...
    const initialVerifyResults = await Promise.all(
      initialClaims.map((c) => verifyClaim(c))
    );
    ...
    for (let j = 0; j < claims.length; j++) {
      ...
      if (c.sourceField === "findingRefs" && c.type === "file_exists") {
        const ref = (rec.findingRefs ?? []).find((x) =>
          String(x).startsWith(c.file + ":")
        );
        if (ref) {
    ...
    const quality = gradeRecommendation(sanitizedRec, {
      knownFindings: recKnownFindings,
    });
  ...
  for (const g of recsGraded) {
    ...
    const claimsWithResults = g.verifyResults.map((r, j) => ({
      ...r,
      claim: g.claims[j],
    }));
    const contradictions = claimsWithResults.filter(
      (r) =>
        ...
        r.claim?.type === "does_not_contradict_project_config"
    );
  ...
  const withheldRecommendations = recsGraded
    ...
    .map((g) => ({
      ...
      reason: hardRegenIndexes.has(g.index)
        ? (regenPlan.find((p) => p.index === g.index)?.regenTrigger ??
          "verification")
        : qualityDroppedIndexes.has(g.index)
          ? "quality_floor"
          : "needs_review",
  ...
  const summary = {
    ...
    averagePassRate:
      recsGraded.length > 0
        ? round4(
            recsGraded.reduce((s, g) => s + g.verification.passRate, 0) /
              recsGraded.length
          )
        : null,
    averageQuality:
      recsGraded.length > 0
        ? round4(
            recsGraded.reduce((s, g) => s + g.quality.overall, 0) /
              recsGraded.length
          )
        : null,
  };
