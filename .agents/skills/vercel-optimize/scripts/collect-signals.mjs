...
function parseArgs(argv) {
  ...
  return {
    explicitProjectId,
    continueWithoutObservability,
    continueUnsupportedFramework,
  };
}
...
async function main() {
  ...
  if (!frameworkSupport.ok && !continueUnsupportedFramework) {
    writeOutput(
      ...
      },
      {
        usable: true,
        blocker: null,
        detail: "Observability Plus was not checked.",
      },
      frameworkSupport
    );
    return;
  ...
  if (oplus && schema) {
    const count = Array.isArray(schema)
      ? schema.length
  ...
  if (oplus) {
    ...
    log(
      "skipping metric queries (Observability Plus preflight did not confirm access)"
    );
  }
    ...
    observabilityPlusConfig.access === false
      ...
      : metricsCanaryOk
        ? {
            usable: true,
            blocker: null,
            detail: "Observability Plus metrics access check passed.",
          }
    ...
    writeOutput(
      {
        ...
        plan: {
          plan: "uncertain",
          reason:
            "not collected before Observability Plus blocker confirmation",
        },
        project: projectCfg,
        contract: null,
        usage: null,
  ...
  if (usageResult?.ok) {
    ...
    if (
      ...
      const filterResult = filterUsageByProject(
        usage,
        project.projectId,
        projectCfg?.name
      );
  ...
  if (projectCfg?.error) {
    log(
      `project config: failed (${projectCfg.error}) — gates that need it will skip`
    );
  }
  ...
  if (oplus && metricsCanaryOk) {
    log(
      `querying observability metrics (${QUERIES.length} metrics in parallel)…`
    );
    const t0 = Date.now();
...
function writeOutput(
  output,
  oplusDiag,
  frameworkSupport = output.frameworkSupport
) {
...
function sumUsageCosts(usage) {
  ...
  if (Array.isArray(usage.services)) {
    return usage.services.reduce(
      (s, x) => s + (x.billedCost ?? x.cost ?? 0),
      0
    );
  }
  if (Array.isArray(usage.breakdown?.data)) {
    return usage.breakdown.data.reduce((s, d) => {
      if (Array.isArray(d.services)) {
        return (
          s +
          d.services.reduce((ss, x) => ss + (x.billedCost ?? x.cost ?? 0), 0)
        );
      }
...
export function diagnoseObservabilityPlus(metrics, oplusProbe) {
  ...
  if (entries.length === 0) {
    return {
      usable: false,
      blocker: "no_oplus_probe",
      detail: "No metrics were attempted.",
    };
  }
  ...
  const totalRows = successes.reduce(
    (s, m) => s + (Array.isArray(m.rows) ? m.rows.length : 0),
    0
  );
  if (totalRows === 0) {
    return {
