...
async function main() {
  ...
  if (!args.signalsPath) {
    ...
    console.error(
      "       VERCEL_OPTIMIZE_MAX_CANDIDATES env var supported (same values)"
    );
    process.exit(1);
  }
  ...
  const allSeeds = gates.flatMap((g) => {
    try {
      ...
      console.error(
        `[gate-investigations] gate ${g.metadata?.id} threw: ${err.message}`
      );
      return [];
    }
  });
  ...
  const displayAnnotated = deduped.map((candidate) =>
    attachDisplayRoute(candidate, signals)
  );
  ...
  const codeScoped = gateable.filter(
    (c) => !c.disqualified && c.scope !== "account"
  );
  const platformScoped = gateable.filter(
    (c) => !c.disqualified && c.scope === "account"
  );
  ...
  const gated = [
    ...gateable
      ...
      .map((c) => ({
        ...c,
        gatedReason: c.disqualifyReason ?? "disqualified",
      })),
...
function resolveBudget(args) {
  const raw =
    args.maxCandidatesArg ?? process.env.VERCEL_OPTIMIZE_MAX_CANDIDATES;
  if (raw == null || raw === "") {
...
function attachDisplayRoute(candidate, signals) {
  ...
  const routes = (signals.codebase?.routes ?? [])
    ...
    .filter(
      (routePath) => typeof routePath === "string" && routePath.length > 0
    );
  ...
  if (!bestRoute || bestScore <= 0 || bestRoute === candidate.route)
    return candidate;
