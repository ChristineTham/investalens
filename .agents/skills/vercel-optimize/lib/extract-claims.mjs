...
export function extractClaims(rec, ctx = {}) {
  ...
  const projectRootDirectory = normalizeProjectRootDirectory(
    ctx.projectRootDirectory
  );
...
function isCacheCandidate(rec) {
  return /^(?:uncached_route|cache_header_gap):/.test(
    String(rec?.candidateRef ?? "")
  );
}
...
function recommendsSharedCache(rec) {
  const haystack = [
    ...
    .filter(Boolean)
    .join("\n");
  return /\b(?:s-maxage|CDN-Cache-Control|Vercel-CDN-Cache-Control|Cache-Control)\b/i.test(
    haystack
  );
}
...
function mentionsNextStableCacheApi(rec) {
  const haystack = recText(rec);
  return (
    /\bunstable_(?:cacheLife|cacheTag)\b/.test(haystack) ||
...
function mentionsNext16RuntimeCacheApiMismatch(rec) {
  ...
  return (
    /\bunstable_cache\b/.test(haystack) &&
    (/\bRuntime Cache\b/i.test(haystack) ||
...
function mentionsRuntimeCacheWhenCacheComponents(rec) {
  ...
  const citations = asArray(rec?.citations).join("\n");
  return (
    /\b(?:Runtime Cache|@vercel\/functions|getCache\s*\(|setCache\s*\()\b/i.test(
      haystack
    ) || /vercel\.com\/docs\/caching\/runtime-cache/i.test(citations)
  );
}
...
function mentionsNextCacheComponentsStaticShellTarget(rec) {
  const haystack = recText(rec);
  if (
    !/\b(?:cacheComponents|Cache Components|cacheLife|cacheTag|['"`]use cache['"`]|use cache|static shell|pre[- ]?render|prerender)\b/i.test(
      haystack
    )
  ) {
    return false;
...
function mentionsImageResponseHeaders(rec) {
  const haystack = recText(rec);
  return (
    /\bImageResponse\b/.test(haystack) &&
    /\bheaders?\b[\s\S]{0,200}\b(?:Cache-Control|s-maxage|CDN|response)\b|\b(?:Cache-Control|s-maxage|CDN)\b[\s\S]{0,200}\bheaders?\b/i.test(
      haystack
    )
  );
}
...
function mentionsNextImagePriorityRecommendation(rec) {
  const haystack = recText(rec);
  if (!/\b(?:next\/image|<Image\b|Image component|image)\b/i.test(haystack))
    return false;
  if (!/\bpriority\b/i.test(haystack)) return false;
  if (
    /\b(?:deprecated|replace|remove|avoid)\b[^.\n]{0,120}\bpriority\b/i.test(
      haystack
    ) ||
    /\bpriority\b[^.\n]{0,120}\b(?:deprecated|replace|remove|avoid)\b/i.test(
      haystack
    )
  ) {
    return false;
  ...
  return (
    /\b(?:set|add|use|enable|mark|make|turn on|with)\b[^.\n]{0,120}\bpriority\b/i.test(
      haystack
...
function mentionsRouteLevelRevalidate(rec) {
  const haystack = recText(rec);
  return (
    /\bexport\s+const\s+revalidate\b/.test(haystack) ||
    /\broute[- ]level\s+revalidate\b/i.test(haystack) ||
    /\brevalidate\s*(?:=|:)\s*\d+\b[^.\n]{0,120}\b(?:page|layout|route segment|segment export)\b/i.test(
      haystack
    )
  );
}
...
function mentionsExistingCacheTagInvalidation(rec) {
  ...
  if (
    !/\b(?:revalidateTag|updateTag|invalidate|invalidation|revalidation|webhook|CMS|content-sync|content sync|publish|deploy)\b/i.test(
      haystack
    )
  ) {
    return false;
  ...
  return (
    /\b(?:existing|current|already|keep|keeps|preserve|preserves|continue|continues|maintain|maintains|via)\b[\s\S]{0,180}\b(?:revalidateTag|updateTag|invalidate|invalidation|revalidation|event-driven|webhook|CMS|content-sync|content sync|publish|deploy|tags?)\b/i.test(
      haystack
    ) ||
...
function mentionsUnsafeImmutableDynamicRoute(rec) {
  ...
  const routeHandler = files.some((file) =>
    /(?:^|\/)route\.[cm]?[jt]sx?$/.test(String(file))
  );
  const apiRoute = /^cache_header_gap:\/api\//.test(
    String(rec?.candidateRef ?? "")
  );
...
function mentionsAuthSensitiveParallelization(rec) {
  const haystack = recText(rec);
  if (
    !/\b(?:parallelize|Promise\.all|run concurrently|start .* early)\b/i.test(
      haystack
    )
  )
    return false;
  if (
    !/\b(?:auth|authorize|authorization|ownership|owns|owner|private|session|permission|access)\b/i.test(
      haystack
    )
  )
    return false;
  return /\b(?:private|secret|token|registrant|account|user|ticket|payment|session)\w*\b/i.test(
    haystack
  );
}
...
function mentionsParallelizationImpactOverclaim(rec) {
  ...
  if (
    ...
    return false;
  return /\b(?:drop|drops|reduce|reduces|reduction|save|saves|shave|shaves)\b[^.\n]{0,200}\b(?:roughly|approximately|about|around|equal\s+to)?\s*(?:the\s+)?(?:duration\s+of\s+[A-Za-z_$][\w$]*\s*\(\s*\)|min\s*\([^)]*duration[^)]*\)|one\s+[\w-]+\s+round[- ]trip|one\s+await|one\s+network\s+call|one\s+database\s+query)/i.test(
    haystack
  );
}
...
function mentionsCpuBoundParallelization(rec) {
  ...
  if (
    ...
    return false;
  return /\b(?:cpu\.p95|CPU p95|cpu p95|CPU-bound|compute-bound|in-process compute|compileMDX|MDX compilation|compilation|render compute)\b/i.test(
    haystack
  );
}
...
function mentionsCachedNotFoundOr404(rec) {
  const haystack = recText(rec);
  if (
    !/\b(?:s-maxage|CDN-Cache-Control|Vercel-CDN-Cache-Control|Cache-Control)\b/i.test(
      haystack
    )
  )
    return false;
  return /\b(?:404|not[- ]found|notFound|not found branch|not-found branch)\b/i.test(
    haystack
  );
}
...
function mentionsIgnoredBuildStepRecommendation(rec) {
  ...
  return (
    ...
    /\b(?:add|set|configure|enable|use|introduce|wire|adopt|turn on)\b[^.\n]{0,180}\b(?:Ignored Build Step|ignoreCommand|turbo-ignore|skip unaffected|unaffected projects?)\b/i.test(
      haystack
    )
  );
}
...
function mentionsTurboBuildCacheRecommendation(rec) {
  const haystack = recText(rec);
  if (
    !/\b(?:Turbo|Turborepo|turbo\.json|tasks\.build|build cache|build caching)\b/i.test(
      haystack
    )
  )
    return false;
  return (
    /\b(?:enable|re-enable|restore|turn on|set|remove)\b[^.\n]{0,220}\b(?:cache\s*:\s*false|tasks\.build\.cache|build cache|build caching|Turbo cache|Turborepo cache)\b/i.test(
      haystack
    ) ||
    /\b(?:cache\s*:\s*false|tasks\.build\.cache|build cache|build caching|Turbo cache|Turborepo cache)\b[^.\n]{0,220}\b(?:enable|re-enable|restore|turn on|set|remove)\b/i.test(
      haystack
    )
  );
}
...
export function summarizeClaimResults(results) {
  ...
  for (const r of results) {
    if (r?.disposition && counts[r.disposition] !== undefined)
