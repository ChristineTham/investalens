...
const HEDGE_WORDS =
  /\b(consider|might|may|could|perhaps|maybe|likely|probably)\b/gi;
const VERB_OPENERS =
...
export function gradeRecommendation(rec, ctx = {}) {
  ...
  const grounding = accountScope
    ? scoreGroundingAccount(rec)
    : scoreGrounding(rec, ctx);
  const evidence = accountScope
    ? scoreEvidenceAccount(rec)
    : scoreEvidence(rec);
...
function scoreSpecificity(rec) {
  ...
  if (Array.isArray(rec.affectedFiles) && rec.affectedFiles.length > 0)
    s += 0.2;
  if (
    Array.isArray(rec.findingRefs) &&
    rec.findingRefs.some((r) => /:\d+/.test(r))
  )
    s += 0.3;
...
function scoreGrounding(rec, ctx) {
  let s = 0;
  const knownFindings = Array.isArray(ctx.knownFindings)
    ? ctx.knownFindings
    : [];
...
function scoreGroundingAccount(rec) {
  let s = 0;
  if (
    typeof rec.candidateRef === "string" &&
    rec.candidateRef.startsWith("platform_")
  )
    s += 0.4;
  else if (typeof rec.candidateRef === "string" && rec.candidateRef.length > 0)
    s += 0.2;
  // Quoting deep-dive data in why/fix is the account-scope equivalent of citing file:line.
  const text = [rec.why, rec.fix, rec.verify]
    .filter((x) => typeof x === "string")
    .join("\n");
  const units = (text.match(UNIT_RE) ?? []).length;
  UNIT_RE.lastIndex = 0;
...
export function applyQualityFloor(recs, floor = 0.55) {
  ...
  for (const rec of recs) {
    ...
    if (o < floor)
      dropped.push({ rec, reason: `quality.overall=${o} < floor=${floor}` });
    else kept.push(rec);
  }
  return { kept, dropped };
}
