...
export function reconcileInvestigation(investigation, { gate = null } = {}) {
  if (!investigation || typeof investigation !== "object") {
    throw new TypeError(
      "reconcileInvestigation investigation must be an object"
    );
  }
  ...
  const rows = arrayAt(candidate, ["evidence", "deepDive", "perDeployment"])
    .filter(
      (row) =>
        ...
        numberValue(row.value) != null
    )
    .map((row) => ({
      deploymentId: row.deployment_id,
      p95: numberValue(row.value),
    }))
...
function routeErrorsMetricMismatchDecision(candidate, ctx) {
  const broadErrors =
    numberAt(candidate, ["evidence", "count"]) ??
    parseSignalNumber(candidate.o11ySignal, "errs");
  ...
  const statusRows = arrayAt(candidate, [
    "evidence",
    "deepDive",
    "errorStatusPattern",
  ]);
  const status5xx = sumRows(statusRows, (row) =>
    /^5\d\d$/.test(String(row?.http_status ?? ""))
  );
...
function uncachedRouteMethodDecision(candidate, ctx) {
  ...
  const gets = sumRows(
    rows,
    (row) => String(row?.request_method ?? "").toUpperCase() === "GET"
  );
...
function isrOverrevalidationDecision(candidate, ctx) {
  const writeRows = arrayAt(candidate, [
    "evidence",
    "deepDive",
    "writePattern",
  ]);
