...
export const metadata = {
  ...
  threshold:
    "fluid=false AND (any cold_start signal OR any route with p95>1000ms AND inv>1000)",
  ...
  return [
    {
      ...
      question:
        "Would enabling Fluid Compute reduce cold-start and warm-instance reuse overhead for the observed hot routes?",
      evidence: {
        fluidEnabled,
        highColdRoutes: cold.slice(0, 5),
        slowHotRoutes: slow.slice(0, 5),
      },
    },
  ];
}
...
function extractHighColdRoutes(signals) {
  ...
  if (
    Array.isArray(live) &&
    live.some((r) => "coldCount" in r || "coldPct" in r)
  ) {
    return live.filter(
      (r) => r.route && (r.coldPct ?? 0) > 0.3 && (r.total ?? 0) > 100
    );
  }
