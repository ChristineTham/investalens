...
export function renderReport({
  ...
  if (sorted.length === 0) {
    ...
    top.forEach((rec, i) => {
      ...
      lines.push(
        `   - **Impact**: ${formatRecommendationText(impactString(rec, signals))}`
      );
...
function renderServiceCostRows(
  ...
  if (hasRealUsage) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      lines.push(
        `| ${escape(s.name ?? "(unnamed)")} | ${escape(usageCells[i])} | ${cost} |`
      );
    }
  } else {
    ...
    if (refs.length > 0) {
      const suffix =
        appliesAlsoTo.length > refs.length
          ...
          : "";
      lines.push(
        `_Also applies to: ${refs.map(displayCandidateRef).join(", ")}${suffix}._`
      );
      lines.push("");
    }
  }
