...
const SERVICE_DIMENSION = [
  ...
  {
    match: /^Observability Events$/i,
    dim: "observability-events",
    family: "observability-events",
  },
  ...
  {
    match: /^Preview Deployment Suffix$/i,
    dim: null,
    family: "fixed",
    actionable: false,
  },
  {
    match: /^Rolling Releases$/i,
    dim: null,
    family: "fixed",
    actionable: false,
  },
  {
    match: /^Observability Plus$/i,
    dim: null,
    family: "fixed",
    actionable: false,
  },
  {
    match: /^Platform Customer Usage$/i,
    dim: null,
    family: "fixed",
    actionable: false,
  },
  {
    match: /^Advanced Deployment Protection$/i,
    dim: null,
    family: "fixed",
    actionable: false,
  },
...
export function computeCostCoverage(usage, gates) {
  ...
  const uncoveredByFamily = [...byFamily.values()]
    ...
    .map((f) => ({
      ...f,
      services: f.services.sort((a, b) => b.billed - a.billed),
    }));
  ...
  return {
    totalBilled: total,
    coveredBilled: covered,
    uncoveredBilled: uncovered,
    uncoveredByFamily,
    topGaps,
  };
}
...
export function renderCostCoverageMarkdown(coverage) {
  if (
    !coverage ||
    !Number.isFinite(coverage.totalBilled) ||
    coverage.totalBilled <= 0
  )
    return [];
  ...
  const uncoveredPct =
    totalBilled > 0 ? (uncoveredBilled / totalBilled) * 100 : 0;
  lines.push(
  ...
  for (const g of actionableGaps) {
    lines.push(
      `| ${escapeCell(g.name)} | $${g.billed.toFixed(2)} | ${(g.share * 100).toFixed(1)}% | ${g.family} | _not analyzed in this run_ |`
    );
  }
  lines.push("");
  lines.push(
    "_Recommendations in this report address the covered dimensions. The uncovered rows are not ignored; they need a separate investigation before we can make safe recommendations._"
  );
  return lines;
}
