export function splitCustomerSafeObservations(
  observations,
  abstentions = [],
  signals = {}
) {
  ...
  for (const observation of Array.isArray(observations) ? observations : []) {
    const unsafeReason = unsupportedObservationReason(
      observation,
      abstentions,
      signals
    );
...
function evidenceText(observation) {
  return [observation?.summary, observation?.evidence]
    .filter(Boolean)
    .join(" ");
}
...
function hasUnsupportedWafBotCategoryClaim(observation) {
  ...
  return (
    /\bbot_category\s*=/i.test(text) ||
    /\btarget(?:ing)?\s+(?:browser_impersonation|automated_browser|ecommerce|monitor)\b/i.test(
      text
    )
  );
}
...
function hasUnsafeBotProtectionObservation(observation) {
  const text = observationText(observation);
  if (
    !/\b(?:Bot Protection|BotID|bot_filter|WAF|managed bot rules?)\b/i.test(
      text
    )
  )
    return false;
  const recommendsAction =
    /\b(?:enable|add|create|configure|challenge|deny|block|rate-limit|target)\b/i.test(
      String(observation?.suggestedAction ?? "")
    );
  if (!recommendsAction) return false;
  const hasSafeRollout = /\b(?:staged|log mode|log action|dry run)\b/i.test(
    text
  );
...
function hasStaleNextCacheApiObservation(observation, signals = {}) {
  ...
  const major = parseInt(
    String(signals?.stack?.frameworkVersion ?? "").match(/\d+/)?.[0] ?? "",
    10
  );
...
function hasImplementationGradeObservationAction(observation) {
  ...
  if (
    /\b(?:use cache:\s*remote|unstable_cache|Cache-Control|s-maxage|cacheLife|export const revalidate|checkBotId|BotID)\b/i.test(
      action
    )
  )
    return true;
  return (
    /\b(?:enable|add|wrap|apply|move|parallelize|set|create|configure|deny|challenge|block|fix|replace|refactor|rewrite|upgrade|downgrade)\b/i.test(
      action
    ) ||
    /\bcache\s+(?:the|this|that|shared|public|origin|response|route|data|lookup|fetch|helper)\b/i.test(
      action
    ) ||
    /\bturn\s+(?:on|off)\b/i.test(action) ||
    /\bswitch\s+(?:to|from|the|this)\b/i.test(action) ||
    /\buse\s+Promise\.all\b/i.test(action) ||
    /\b(?:raise|lower|increase|decrease)\s+(?:the\s+)?(?:TTL|timeout|memory|CPU|cache|cache lifetime|duration)\b/i.test(
      action
    )
  );
}
...
function contradictsNoChangeReason(observation, abstentions) {
  ...
  if (
    /\bparalleliz(?:e|ing)\b/.test(lowerObservationText) &&
    /\bgetsession\b/.test(lowerObservationText) &&
    relevantReasons.some(
      (reason) =>
        /\bgetsession\b/.test(reason) &&
        /\b(?:gates?|redirect|auth-preserving|blocked)\b/.test(reason)
    )
  ) {
    return true;
  }
...
function hasUnsupportedFrameworkCausalClaim(observation) {
  ...
  return (
    ...
    /next\.js\s+\d+(?:\.\d+)?[^.]{0,120}treats[^.]{0,120}dynamic api/.test(
      text
    ) ||
...
function hasUnsupportedStaticGenerationClaim(observation) {
  ...
  return (
    /\b(?:returns?\s*(?:an\s+)?empty|\[\])\b[^.\n]{0,240}\b(?:every request|on[- ]demand|no params? (?:are )?prebuilt|populate generatestaticparams|served from (?:the )?cdn|hit bucket|cachebreakdown)\b/i.test(
      text
    ) ||
    /\b(?:every request|on[- ]demand|no params? (?:are )?prebuilt|populate generatestaticparams|served from (?:the )?cdn|hit bucket|cachebreakdown)\b[^.\n]{0,240}\b(?:returns?\s*(?:an\s+)?empty|\[\])\b/i.test(
      text
    ) ||
    /\bdynamic\s*=\s*['"`]error['"`]\b[^.\n]{0,240}\b(?:generatestaticparams|dynamicparams|every request|on[- ]demand)\b/i.test(
      text
    )
  );
}
...
function hasUnsupportedSourceAbsenceClaim(observation) {
  ...
  const text = observationText(observation).toLowerCase();
  return /\b(?:enoent|no\s+(?:matching|corresponding)\s+(?:mdx|file|post)|missing\s+(?:mdx|file|post)|does\s+not\s+exist|not\s+found\s+on\s+disk)\b/.test(
    text
  );
}
...
export function hasUnsupportedCacheLifeCdnText(text) {
  ...
  return (
    /\bcacheLife\b[^.\n]{0,240}\b(?:Cache-Control|s-maxage|CDN|edge cache|cache breakdown|x-vercel-cache|HIT|MISS|function (?:still )?runs per request|every request invokes the function|canonical|toLaunch-\d+)\b/i.test(
      text
    ) ||
...
function hasUnsupportedRuntimeRootCauseClaim(observation) {
  const text = observationText(observation);
  if (
    !/\b(?:caused by|root cause|responsible for failures|would produce)\b/i.test(
      text
    )
  )
    return false;
  if (!/\b(?:5xx|500|error|failures?)\b/i.test(text)) return false;
  return !/\b(?:logs?\s+(?:show|confirm|include|contain)|stack\s+(?:shows|trace|evidence)|trace\s+(?:shows|confirms)|exception\s+(?:shows|confirms)|response body\s+(?:shows|confirms)|runtime evidence)\b/i.test(
