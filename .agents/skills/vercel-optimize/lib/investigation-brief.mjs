...
export function inferPlaybook(signals) {
  ...
  const services = Array.isArray(signals?.usage?.services)
    ? signals.usage.services
    : [];

  const has = (name) => Boolean(deps[name]);
  const hasPrefix = (prefix) =>
    Object.keys(deps).some((k) => k === prefix || k.startsWith(prefix));
  const anyRouteMatches = (re) => routePaths.some((p) => re.test(p));
  const usageHas = (re, minBilled = 0) =>
    services.some(
      (s) =>
        re.test(String(s?.name ?? "")) &&
        Number(s?.billedCost ?? s?.cost ?? 0) > minBilled
    );
  ...
  if (
    routes.length > 0 &&
    routes.every((r) => /^\/api\//.test(r.routePath ?? ""))
  ) {
    return "api-service";
  }
...
export function resolveFiles(candidate, signals) {
  ...
  const workspaceImports = matched
    .flatMap((r) =>
      Array.isArray(r.workspaceImports) ? r.workspaceImports : []
    )
    .filter(Boolean);
  return capBriefFiles(
    uniq([...direct, ...workspaceImports]),
    layoutFiles,
    routes
  );
}
...
export function routePathMatchScore(routePath, metricPath) {
  ...
  if (ri !== rTokens.length || mi !== mTokens.length) {
    return trailingSingleDynamicPartialScore(
      rTokens,
      mTokens,
      ri,
      mi,
...
function trailingSingleDynamicPartialScore(
  rTokens,
  mTokens,
  ri,
  mi,
  ...
  if (
    !isSingleDynamicPlaceholder(lastRouteToken) &&
    !isSingleDynamicPlaceholder(lastMetricToken)
  )
    return 0;
...
function briefRoots(signals) {
  ...
  const appRoot =
    typeof codebase.rootDir === "string" && codebase.rootDir.length > 0
      ...
      : null;
  const repoRoot =
    typeof codebase.monorepoRoot === "string" &&
    codebase.monorepoRoot.length > 0
  ...
  return routes
    .filter(
      (r) =>
        r.type === "layout" &&
        r.file &&
        layoutAppliesToRoute(r.routePath, route)
    )
    .sort(
      (a, b) =>
...
function isLayoutPath(file) {
  return /(^|\/)(?:\+layout(?:\.server)?|layout)\.(?:svelte|tsx?|jsx?)$/.test(
    String(file ?? "")
  );
}
...
export function buildBrief({
  ...
  const candidateRef =
    candidate.candidateRef ?? `${kind}:${routeOrHost ?? "<account>"}`;
  const roots = briefRoots(signals);

  const lines = [];
  lines.push(
    `# Investigation brief — ${kind}${routeOrHost ? ` — ${routeOrHost}` : ""}`
  );
  lines.push("");
  lines.push(
  ...
  if (candidate.disqualified) {
    lines.push(
      `- **⚠ Disqualifier present:** ${candidate.disqualifyReason ?? "disqualified"}`
    );
  }
  lines.push("");
  lines.push(
  ...
  if (Array.isArray(files) && files.length > 0) {
    ...
    const routeScores = routes
      ...
      .filter((x) => x.score > 0);
    const topScore =
      routeScores.length > 0 ? Math.max(...routeScores.map((x) => x.score)) : 0;
    ...
    const layoutFiles = new Set(
      closestAncestorLayoutFiles(routeOrHost, routes)
    );
  ...
  if (frameworkPlaybookId && frameworkPlaybookBody) {
    ...
    lines.push("");
    lines.push(
      `_Framework-shaped advice for ${framework}. Same rule: evidence-grounded only._`
    );
    lines.push("");
  }
  ...
  lines.push(
    "4. Stop after 5 files exhausted, or when you have a verified root cause."
  );
  lines.push(
  ...
  lines.push(
    "- Every citation appears in the library above. No invented URLs."
  );
  lines.push(
    "- `currentBehavior` snippet appears in the actual file (not a paraphrase)."
  );
  lines.push("- No `$N` dollar literals in any customer-facing field.");
  lines.push("");
  lines.push(
    "If ANY of these fails, fix the rec OR switch to Outcome B (abstain)."
  );
  lines.push("");
